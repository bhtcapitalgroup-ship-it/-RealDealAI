...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "location",
            sa.Column(
                "location", sa.Text(), nullable=True
            ),  # Will be cast — see raw SQL below
        ),
        # Listing info
    ...
    op.execute("ALTER TABLE properties ADD COLUMN location geography(Point, 4326)")
    ...
    op.create_index(
        "ix_properties_investment_score",
        "properties",
        ["investment_score"],
        postgresql_using="btree",
    )
    op.create_index(
        "ix_properties_cap_rate", "properties", ["cap_rate"], postgresql_using="btree"
    )
    ...
    op.create_index(
        "ix_properties_mls_number", "properties", ["mls_number"], unique=True
    )
    ...
    op.create_unique_constraint(
        "uq_saved_deals_user_property", "saved_deals", ["user_id", "property_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "channel", alert_channel_enum, nullable=False, server_default="email"
        ),
        ...
        # }
        sa.Column(
            "frequency", sa.String(50), nullable=False, server_default="instant"
    ...
    op.create_table(
        ...
        sa.Column(
            "price_change_yoy", sa.Numeric(6, 3), nullable=True
        ),  # year over year %
        ...
        sa.Column(
            "trend_direction", sa.String(20), nullable=True
    ...
    op.create_index(
        "ix_market_data_period", "market_data", ["period_start", "period_end"]
    )
    op.create_unique_constraint(
