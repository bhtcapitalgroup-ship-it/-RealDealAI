...
async def generate_verdict(
    ...
    prompt = _build_prompt(
        property_data, rent_estimate, metrics, neighborhood, investment_score
    )
...
def _rule_based_verdict(
    ...
    if cap < 5:
        risks.append(
            f"Low cap rate ({cap:.1f}%) — below the 5% minimum for positive leverage"
        )
    if cf < 0:
        risks.append(
            f"Negative cash flow (${cf:,.0f}/mo) — you'd be subsidizing this property"
        )
    ...
    if hood and hood.get("rent_growth", 0) > 4:
        opps.append(
            f"Strong rent growth ({hood['rent_growth']:.1f}%/yr) suggests rising income"
        )
    if hood and hood.get("school_rating", 0) >= 8:
        opps.append(
            f"Top-rated schools ({hood['school_rating']}/10) attract quality tenants"
        )
    if not opps:
    ...
    if verdict == "Good Deal":
        summary_parts.append(
            f"This property scores {score}/100 with strong fundamentals."
        )
    elif verdict == "Average":
        summary_parts.append(f"This property scores {score}/100 with mixed signals.")
    else:
        summary_parts.append(
            f"This property scores {score}/100 and doesn't meet investment thresholds."
        )

    if cf > 0:
        summary_parts.append(
            f"Positive cash flow of ${cf:,.0f}/mo with {cap:.1f}% cap rate."
        )
    else:
        summary_parts.append(
            f"Negative cash flow of ${cf:,.0f}/mo makes this a risky hold."
        )
